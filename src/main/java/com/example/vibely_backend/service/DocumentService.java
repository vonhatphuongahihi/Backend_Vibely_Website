...
public class DocumentService {
    ...
    public ApiResponse createLevel(String name) {
        if (name == null || name.trim().isEmpty()) {
            return new ApiResponse(400, "Tên cấp học không được để trống", null);
        }
        if (levelRepository.findByName(name).isPresent()) {
            return new ApiResponse(400, "Cấp học đã tồn tại", null);
        }

        Level level = new Level();
        level.setName(name);
        levelRepository.save(level);
        return new ApiResponse(201, "Tạo cấp học thành công", level);
    }

    public ApiResponse createSubject(String name, String levelId) {
        if (name == null || levelId == null) {
            return new ApiResponse(400, "Tên môn học và cấp học là bắt buộc", null);
        }
        Optional<Level> levelOpt = levelRepository.findById(levelId);
        if (levelOpt.isEmpty()) {
            return new ApiResponse(404, "Không tìm thấy cấp học", null);
        }

        Level level = levelOpt.get();

        if (subjectRepository.findByNameAndLevel(name, level).isPresent()) {
            return new ApiResponse(400, "Môn học đã tồn tại cho cấp học này", null);
        }
        ...
        return new ApiResponse(201, "Tạo môn học thành công", subject);
    }

    public ApiResponse getAllLevels() {
        List<Level> levels = levelRepository.findAll();
        return new ApiResponse(200, "Lấy danh sách cấp học thành công", levels);
    }
    ...
    public ApiResponse getSubjectsByLevel(String levelId) {
        ...
        return new ApiResponse(200, "Lấy danh sách môn học thành công", result);
    }
    ...
    public ApiResponse createDocument(DocumentRequest request) {
        ...
        if (levelOpt.isEmpty()) 
            return new ApiResponse(400, "Cấp học không hợp lệ", null);
        
        Optional<Subject> subjectOpt = subjectRepository.findById(request.getSubjectId());
        if (subjectOpt.isEmpty()) 
            return new ApiResponse(400, "Môn học không hợp lệ", null);
        ...
        return new ApiResponse(201, "Tạo tài liệu thành công", response);
    }
    ...
    public ApiResponse getFilteredDocuments(String query, String levelId, String subjectId) {
        ...
        return new ApiResponse(200, "Lấy danh sách tài liệu thành công", documents);
    }
    ...
    public ApiResponse getDocumentById(String id) {
        ...
        return new ApiResponse(200, "Lấy tài liệu theo ID thành công", response);
    }
    ...
    public ApiResponse updateDocument(String id, DocumentRequest req) {
        ...
        return new ApiResponse(200, "Cập nhật tài liệu thành công", response);
    }

    public ApiResponse deleteDocument(String id) {
        if (!documentRepository.existsById(id)) {
            return new ApiResponse(404, "Không tìm thấy tài liệu", null);
        }
        documentRepository.deleteById(id);
        return new ApiResponse(200, "Xóa tài liệu thành công", null);
    }
    ...
    public ApiResponse saveDocument(String userId, String documentId) {
        ...
        if (user.getSavedDocuments().contains(document)) {
            return new ApiResponse(400, "Tài liệu đã được lưu trước đó", null);
        }
    
        user.getSavedDocuments().add(document);
        userRepository.save(user);
    
        return new ApiResponse(200, "Lưu tài liệu thành công", user.getSavedDocuments());
    }
